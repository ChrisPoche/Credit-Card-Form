...
class App extends React.Component {
    ...
    componentDidMount() {
        window.addEventListener('keypress', (e) => {
            if (e.srcElement.name === 'card-number' || e.srcElement.name === 'card-cvv') {
                ...
                if ((e.code.slice(0, 5) !== 'Digit' && e.code.slice(0, 6) !== 'Numpad') || (e.code.slice(0, 6) === 'Numpad' && e.code.length > 7) || e.shiftKey === true) e.preventDefault();
            }
            else if (e.srcElement.name === 'card-name') {
                if ((e.keyCode > 64 && e.keyCode < 91) || (e.keyCode > 96 && e.keyCode < 123) || e.keyCode === 32 ||  e.keyCode === 45 || e.keyCode === 13 || e.keyCode === 8) this.setError('');
                else if (e.keyCode === 13) this.onSubmit();
                else {
                    e.preventDefault();
    ...
    onCardChange = (val) => {
        ...
        if (val.target.id === 'card-exp-month-form') {
            let now = new Date();
            (now.getMonth() + 1 > parseInt(val.target.value) && parseInt(this.state.expYear) === now.getFullYear()) ? this.setState({ expYear: '', expMonth: val.target.value }) : this.setState({ expMonth: val.target.value });
            this.resizeFocus();
        };
